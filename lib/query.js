...
Query.prototype._mergeJoinScope = function(joinScope, options) {
  joinScope = makeJoinOptions(joinScope)
  var result = Object.assign({}, joinScope, {
    _rel: joinScope._rel,
    _apply: util.compose(options._apply, joinScope._apply),
    _applyPre: util.compose(options._applyPre, joinScope._applyPre),
    _array: (joinScope._array === false || options._array === false) ? false : true,
  })
  var subOptions = Object.assign({}, options)
  delete subOptions._rel
  delete subOptions._apply;
  delete subOptions._applyPre;
  if (joinScope._optionsOn) {
    result[joinScope._optionsOn] = subOptions;
  } else {
    Object.assign(result, subOptions)
  }
  var throughJoin = this._model._joinScopes[joinScope._rel] 
  if (throughJoin) {
    return this._mergeJoinScope(throughJoin, result)
  } else {
    return result
  }
} 

Query.prototype._getOneJoin = function(key, options, getAll, gotModel) {
  options = makeJoinOptions(options);
  var model = this._model
  var r = model._getModel()._thinky.r;
  var joinScope = model._getModel()._joinScopes[key];

  if (joinScope) {
    options = this._mergeJoinScope(joinScope, options)
  }

  var relation = options._rel || key
  var join = model._getModel()._joins[relation]
    ...
    this._query = this._query.merge(function(doc) {
      return doc.hasFields(join.leftKey).branch(
        queryForJoin(doc, model, key, join,
          function(subQuery) {
            if (options._applyPre) {
              subQuery = options._applyPre(subQuery);
            }

            if (options._array !== false &&
                join.type !== 'hasOne' && 
                join.type !== 'belongsTo') {
              subQuery = subQuery.coerceTo("ARRAY");
            }

            subQuery = subQuery.getJoin(options, getAll, gotModel)._query;

            if (options._apply) {
              subQuery = options._apply(subQuery);
            }
...
Query.prototype.getJoin = function(modelToGet, getAll, gotModel) {
  ...
  if (getAll) {
    ...
    util.loopKeys(joins, function(joins, key) {
      if (!gotModel[joins[key].model.getTableName()]) {
        self._getOneJoin(key, modelToGet[key], getAll, gotModel)
      }
    });
  } else {
